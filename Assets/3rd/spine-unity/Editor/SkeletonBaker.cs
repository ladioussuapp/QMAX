...
public static class SkeletonBaker {
	...
	public static void GenerateMecanimAnimationClips (SkeletonDataAsset skeletonDataAsset, JSONNode jsonConfig = null) {
        ...
        Dictionary<string, AnimationClip> animclipTable = new Dictionary<string, AnimationClip>();
        Dictionary<string, AnimationClipSettings> acsTable = new Dictionary<string, AnimationClipSettings>();
        /*****************************************************/
        if (jsonConfig != null) {
            JSONListFieldValue setts = (JSONListFieldValue)jsonConfig.GetField("settings");
            if (setts != null) {
                foreach (var obj in setts.value) {
                    string animationName;
                    bool loopTime;

                    JSONInStream inStream = new JSONInStream(obj.Serialize());
                    inStream.Content("Animation", out animationName).Content("LoopTime", out loopTime);
                    AnimationClipSettings acs = new AnimationClipSettings();
                    acs.loopTime = loopTime;
                    acsTable.Add(animationName, acs);
                }
            }
        }
        /*****************************************************/
		...
		foreach (var anim in data.Animations) {
            ...
            if (acsTable.ContainsKey(name)) {
                var cfs = acsTable[name];
                settings.loopTime = cfs.loopTime;
            }
        ...
        if (jsonConfig != null)
        {
            JSONListFieldValue parameters = (JSONListFieldValue)jsonConfig.GetField("parameters");
            JSONListFieldValue controllers = (JSONListFieldValue)jsonConfig.GetField("controllers");
            JSONListFieldValue events = (JSONListFieldValue)jsonConfig.GetField("events");
            if (events != null)
            {
                foreach (var obj in events.value)
                {
                    string animationName, functionName, parameter;
                    float frameIndex, frameCount;
                    JSONInStream inStream = new JSONInStream(obj.Serialize());
                    inStream.Content("Animation", out animationName)
                        .Content("Function", out functionName)
                        .Content("Parameter", out parameter)
                        .Content("FrameIndex", out frameIndex)
                        .Content("FrameCount", out frameCount);
                    if (animclipTable.ContainsKey(animationName))
                    {
                        AnimationClip animation = animclipTable[animationName];
                        
                        AnimationEvent[] res = UnityEditor.AnimationUtility.GetAnimationEvents(animation);
                        AnimationEvent[] animEvent = null;
                        if (res != null)
                        {
                            animEvent = new AnimationEvent[res.Length + 1];
                        ...
                        {
                            animEvent = new AnimationEvent[1];
                        }
                        animEvent[animEvent.Length - 1] = new AnimationEvent();
                        animEvent[animEvent.Length - 1].functionName = functionName;
                        animEvent[animEvent.Length - 1].stringParameter = parameter;
                        animEvent[animEvent.Length - 1].time = frameIndex / frameCount;

                        UnityEditor.AnimationUtility.SetAnimationEvents(animation, animEvent);
                    }
                }
            }
            if (controllers != null && parameters != null)
            {
                Dictionary<string, UnityEditor.Animations.AnimatorState> animStateTable = new Dictionary<string, UnityEditor.Animations.AnimatorState>();
                UnityEditor.Animations.AnimatorControllerLayer layer = controller.layers[0];
                layer.stateMachine.AddStateMachineBehaviour<BaseStateMachineBehaviour>();

                // parameters
                foreach (var obj in parameters.value)
                {
                    string name, type;
                    JSONInStream inStream = new JSONInStream(obj.Serialize());
                    inStream.Content("Name", out name).Content("Type", out type);

                    UnityEngine.AnimatorControllerParameter acp = new UnityEngine.AnimatorControllerParameter();
                    acp.name = name;
                    if (type == "Int") {
                        acp.type = UnityEngine.AnimatorControllerParameterType.Int;
                        int value;
                        inStream.Content("Value", out value);
                        acp.defaultInt = value;
                    } else if (type == "Float") {
                        acp.type = UnityEngine.AnimatorControllerParameterType.Float;
                        float value;
                        inStream.Content("Value", out value);
                        acp.defaultFloat = value;
                    } else if (type == "Boolean") {
                        acp.type = UnityEngine.AnimatorControllerParameterType.Bool;
                        bool value;
                        inStream.Content("Value", out value);
                        acp.defaultBool = value;
                    } else if (type == "Trigger") {
                        acp.type = UnityEngine.AnimatorControllerParameterType.Trigger;
                        bool value;
                        inStream.Content("Value", out value);
                        acp.defaultBool = value;
                    }
                    controller.AddParameter(acp);
                }
                // controllers
                foreach (var obj in controllers.value)
                {
                    string form, to;
                    bool hasExitTime;
                    float txitTime, duratio, offset;
                    JSONInStream inStream = new JSONInStream(obj.Serialize());
                    inStream.Content("From", out form)
                        .Content("To", out to)
                        .Content("HasExitTime", out hasExitTime)
                        .Content("ExitTime", out txitTime)
                        .Content("TransitionDuratio", out duratio)
                        .Content("TransitionOffset", out offset);
                    // conditions
                    JSONNode tmp = ((JSONObjectFieldValue)obj).value;
                    JSONListFieldValue conditions = (JSONListFieldValue)tmp.GetField("Conditions");

                    UnityEditor.Animations.AnimatorStateTransition transition = null;
                    if (animclipTable.ContainsKey(form) && animclipTable.ContainsKey(to))
                    {
                        UnityEditor.Animations.AnimatorState formState = null;
                        UnityEditor.Animations.AnimatorState toState = null;
                        if (!animStateTable.ContainsKey(form))
                        {
                            formState = AddStateTransition(animclipTable[form], layer);
                            animStateTable.Add(form, formState);
                        }
                        else
                        {
                            formState = animStateTable[form];
                        }

                        if (!animStateTable.ContainsKey(to))
                        {
                            toState = AddStateTransition(animclipTable[to], layer);
                            animStateTable.Add(to, toState);
                        }
                        else
                        {
                            toState = animStateTable[to];
                        }

                        transition = formState.AddTransition(toState);
                        transition.hasExitTime = hasExitTime;
                        transition.exitTime = txitTime;
                        transition.duration = duratio;
                        transition.offset = offset;
                    }
                    else if (animclipTable.ContainsKey(form) && to.Equals("Exit"))
                    {
                        UnityEditor.Animations.AnimatorState formState = null;
                        if (!animStateTable.ContainsKey(form))
                        {
                        ...
                        {
                            formState = animStateTable[form];
                            transition = formState.AddExitTransition(hasExitTime);
                            transition.exitTime = txitTime;
                            transition.duration = duratio;
                            transition.offset = offset;
                        }
                    ...
                    if (transition != null)
                    {
                        foreach (var cond in conditions.value)
                        {
                            string parameter, mode;
                            float threshold;
                            JSONInStream condStream = new JSONInStream(cond.Serialize());
                            condStream.Content("Parameter", out parameter).Content("Mode", out mode).Content("Threshold", out threshold);

                            if (mode == "If") {
                                transition.AddCondition(UnityEditor.Animations.AnimatorConditionMode.If, threshold, parameter);
                            } else if (mode == "IfNot") {
                                transition.AddCondition(UnityEditor.Animations.AnimatorConditionMode.IfNot, threshold, parameter);
                            } else if (mode == "Greater") {
                                transition.AddCondition(UnityEditor.Animations.AnimatorConditionMode.Greater, threshold, parameter);
                            } else if (mode == "Less") {
                                transition.AddCondition(UnityEditor.Animations.AnimatorConditionMode.Less, threshold, parameter);
                            } else if (mode == "Equals") {
                                transition.AddCondition(UnityEditor.Animations.AnimatorConditionMode.Equals, threshold, parameter);
                            } else if (mode == "NotEqual") {
                                transition.AddCondition(UnityEditor.Animations.AnimatorConditionMode.NotEqual, threshold, parameter);
                            }
                        }
                    }
                }
    ...
    private static UnityEditor.Animations.AnimatorState AddStateTransition(AnimationClip clip, UnityEditor.Animations.AnimatorControllerLayer layer)
    {
