...
public class SkeletonAnimator : SkeletonRenderer, ISkeletonAnimation {
    ...
    /// <summary>
    /// 默认跳帧率
    /// 比如设置为3，那么就每3次Update才执行一次
    /// </summary>
    public static int DefaultSkipRate = 2;
 
    /// <summary>
    /// 此spine 的调整率     小于0表示使用默认值 DefaultSkipRate ，等于0表示不跳帧
    /// </summary>
    public int CurrentSkipRate = -1;
    ...
    void Start()
    {
        if (CurrentSkipRate < 0)
        {
            //没有单独设置此spine的跳帧
            CurrentSkipRate = DefaultSkipRate;
        }

        //这里设置随机数，是如果多个SkeletonAnimator同时初始化时，可以让他们的Update错开
        //不会让所有Update集中在同一帧
        if (skipUpdateCount == -1)
            skipUpdateCount = Random.Range(0, CurrentSkipRate);
    }
	...
	void Update () {
        ...
        if (++skipUpdateCount < CurrentSkipRate)
            return;
