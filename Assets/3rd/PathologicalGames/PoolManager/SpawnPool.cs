...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        public void CreatePrefabPool(PrefabPool prefabPool, string key)
        {
            // Only add a PrefabPool once. Uses a GameObject comparison on the prefabs
            //   This will rarely be needed and will almost Always run at game start, 
            //   even if user-executed. This really only fails If a user tries to create 
            ...
            //在poolmanager中做了检测key的处理， 当key不同但是 prefabPool.prefab 相同时，还是要创建一个新的pool。
            //bool isAlreadyPool = this.GetPrefabPool(prefabPool.prefab) == null ? false : true;
            //if (!isAlreadyPool)
            //{
                // Used internally to reference back to this spawnPool for things 
                ...
                // Add to the prefabs dict for convenience
                this.prefabs._Add(key, prefabPool.prefab);
            //}
        ...
        public void DesPrefabPool(PrefabPool prefabPool)
        {
            bool isAlreadyPool = this.GetPrefabPool(prefabPool.prefab) == null ? false : true;

            if (!isAlreadyPool)
            {
                Debug.LogError("PrefabPool not found");
            }

            this._prefabPools.Remove(prefabPool);
            this.prefabs._Remove(prefabPool.prefab);
            Transform t;

            for (int i = 0; i < prefabPool._despawned.Count; i ++ )
            {
                t = prefabPool._despawned[i];
        ...
        //這種設定為所有播放同一種音樂的go設為同一個prefab。 
        //應該為所有添加了AudioSource組件的物體都為同一個prefab，然後設置不同的clip，播放。
        public AudioSource Spawn(AudioSource prefab,
                            	 Vector3 pos, Quaternion rot,
                            	 Transform parent)
        {
    ...
    public class PrefabPool
    {
        ...
        public Transform SpawnNew(Vector3 pos, Quaternion rot)
        {
            ...
            if (!this.spawnPool.dontReparent)
            {
                ...
                {
                    (inst as RectTransform).SetParent(this.spawnPool.group);
                }
                else
                {
    ...
    public class PrefabsDict : IDictionary<string, Transform>
    {
        ...
        internal bool _Remove(Transform t)
        {
            foreach (KeyValuePair<string,Transform> keyVal in this._prefabs)
            {
                if (t == keyVal.Value)
                {
                    this._prefabs.Remove(keyVal.Key);
                    return true;
                }
            }
