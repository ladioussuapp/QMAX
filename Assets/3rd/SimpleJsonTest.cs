...
class SimpleJsonTest : MonoBehaviour
{
    ...
    void Test()
    {
        var N = SimpleJSON.JSONNode.Parse("{\"name\":\"test\", \"array\":[1,{\"data\":\"value\"}]}");
        N["array"][1]["Foo"] = "Bar";
        ...
        N = SimpleJSON.JSONNode.LoadFromBase64(data);
        P("Deserialized from Base64 string:");
        P(N.ToString());
        P("");

        var I = new SimpleJSON.JSONClass();
        I["version"].AsInt = 5;
